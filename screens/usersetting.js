...
import { Text, View , Button,TouchableOpacity,  keyboardType,Image, TextInput,Modal,Alert,Platform} from 'react-native';
...
const Setting = ()=>{
    ...
    const [memberuuid,setmemuuid] = useState();
...
const saveData = async (key, value) => {
  ...
  const lookuuid = async() => {
    console.log(memberuuid);
    await Clipboard.setStringAsync(memberuuid);
    const text = await Clipboard.getStringAsync();
    Alert.alert('提醒','已將ID複製至剪貼版');
  }
  const uploadImage = async () => {
    ...
    try {
        ...
        formData.append('headimg', {
            uri: images[0],
            type: 'image/jpeg', 
            name: 'image.jpg',
        });
        const response = await fetch("https://lively-nimbus-415015.de.r.appspot.com/api/member/patch/"+ uuid +"/", {
            method: 'PATCH',
            body: formData,
            headers: {
        ...
        <View style={{height:70,backgroundColor:orange,flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
            ...
            <Text  style={{fontSize:20,left:20,color:orange}}>個人設定</Text>
                ...
                <View style={{ height: 10 }}></View>
                        ...
                        <TouchableOpacity onPress={lookuuid} style={{ backgroundColor: '#fff',  flexDirection: 'row', borderBottomLeftRadius:10,borderBottomRightRadius:10,justifyContent:'center', alignItems: 'center',height:60, width: '80%'}}>
                        <Text  style={{fontSize:18,left:10}}>複製ID</Text>
                        <Text style={{ flex: 1, fontSize: 17, color: holderwords ,textAlign:'right',right:10}}>請由機構方教學後操作</Text>
                        </TouchableOpacity>
